...
export default function Login() {
    ...
    const filterErrors = async () => {
        if (!(formState.username && formState.password)) {
            ...
            action(formState.username, formState.password)
                  .then( res =>  res.json())
                  .then( data => {
                      if (data.error) {
                        setFormState({
                    ...
                    <Paper className={classes.root} elevation={3}>
                        <form>
                            ...
                            <div className={classes.loginbutton}>
                                <Button variant="contained" onClick={filterErrors} color="primary">
                                    {formState.login ? "login" : "signup"}
                                </Button>
                            </div>
                            <div>
                                {formState.login ? "Not registered?" : "alread have an account?"}
                            </div>
                            <div>
                                <Button onClick={() => setFormState({ ...formState, login : !formState.login })}>
